...
token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjozLCJ1c2VybmFtZSI6ImFydGh1ciIsImV4cGlyYXRpb24iOiIyMDIzLTA0LTA3IDEyOjA1OjQ0LjA0NTY2OSIsImlzc3VlZF9hdF90aW1lIjoiMjAyMy0wNC0wNiAxMjowNTo0NC4wNDU2NzgifQ.zlmgc7dWGEJM23YmQcTeF-YpDWiZZeSEN6BkrjW0wqA"
...
def _send_bid_request_and_check_response(bid_request_data):
    response = requests.post(RTB_BID_REQUEST_URL, headers=headers, json=bid_request_data)
...
def _send_loss_and_check_response(request_id):
    response = requests.post(RTB_NOTIFY_URL, headers=headers, json={"id": request_id, "win": False})
    assert response.status_code == 200


def _send_win_and_check_response(request_id, bid_response_body):
    response = requests.post(RTB_NOTIFY_URL, headers=headers, json={
        "id": request_id,
        "win": True,
...
def test_configuration():
    ...
    response = requests.post(GAME_CONFIGURATION_URL, headers=headers, json=body)
    assert response.status_code == 200
...
def test_script_game():
    ...
    response = requests.post(CAMPAIGNS_URL, headers=headers, json={"budget": body["budget"], "name": "test_campaign"})
    ...
    response = requests.post(CREATIVES_URL, headers=headers, json={
